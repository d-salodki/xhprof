...
/* {{{ arginfo */
ZEND_BEGIN_ARG_INFO_EX(arginfo_xhprof_enable,
0, 0, 0)
...
ZEND_BEGIN_ARG_INFO(arginfo_xhprof_sample_enable,
0)
...
ZEND_DLEXPORT void hp_execute_internal(zend_execute_data *execute_data, zval *return_value TSRMLS_DC) {
    ...
    func = hp_get_function_name(&current_data->func->op_array
    TSRMLS_CC);
    if (func) {
        BEGIN_PROFILING(&hp_globals.entries, func, hp_profile_flag);
...
*
hp_compile_file(zend_file_handle
*file_handle,
int type
TSRMLS_DC) {
const char *filename;
